...
class ORMBroadcastKey extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'orm:bckey {--show : Display the key instead of modifying files}';
    ...
    {
        if ($this->option('show')) {
            $key = $this->laravel['config']['orm.bc_key'];
    ...
    protected function setBroadcastKeyInEnv($key)
    {
        file_put_contents($this->laravel->environmentFilePath(), str_replace(
            'BC_KEY=' . $this->laravel['config']['orm.bc_key'],
            'BC_KEY=' . $key,
